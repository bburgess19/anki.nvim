...
local function create_floating_window(content, height, width)
...
anki.review = function(deck)
    ...
    local function fetch_cards()
        ...
        if success and result and type(result) == "table" then
            ...
            UTIL.notify_error("Could not fetch due cards: " .. vim.inspect(result))
            cards = {}
        end
    end

    local function review_quit()
        if win and vim.api.nvim_win_is_valid(win) then
            vim.api.nvim_win_close(win, true)
        end
        if anki._review_win and vim.api.nvim_win_is_valid(anki._review_win) then
            vim.api.nvim_win_close(anki._review_win, true)
        end
        win = nil
        buf = nil
        anki._review_win = nil
    ...
    local function show_card()
        if #cards == 0 or not cards[current] then
            if win and vim.api.nvim_win_is_valid(win) then
                vim.api.nvim_win_close(win, true)
            end
            if anki._review_win and vim.api.nvim_win_is_valid(anki._review_win) then
            ...
            anki._review_win = nil
            anki._review_buf = nil
            UTIL.notify_info("No more cards to review!")
            return
        end
        local card_id = cards[current]
        local success, card = api.request2({
            action = "cardsInfo",
            version = 6,
            params = { cards = { card_id } },
        })
        if not success or not card or not card[1] then
            UTIL.notify_error("Could not fetch card info.")
            return
        end
        card_info = card[1]
        local question = card_info.question or "(No question)"
        local lines = {}
        table.insert(lines, "Review Card " .. current .. "/" .. #cards)
        table.insert(lines, string.rep("-", 40))
        for l in question:gmatch("[^\n]+") do
            table.insert(lines, l)
        end
        table.insert(lines, string.rep("-", 40))
        table.insert(lines, "[1] Again   [2] Hard   [3] Good   [4] Easy   [q] Quit")
        buf, win = create_floating_window(lines, 25, 75)
        anki._review_win = win
        anki._review_buf = buf
        vim.keymap.set(
            "n",
            "1",
    ...
    local function review_next()
        current = current + 1
        -- Clear previous window references before showing next card
        if anki._review_win and vim.api.nvim_win_is_valid(anki._review_win) then
            vim.api.nvim_win_close(anki._review_win, true)
        end
        anki._review_win = nil
    ...
    function anki.review_answer(ease)
        ...
        -- Show answer
        local answer = card_info.answer or "(No answer)"
        local lines = {}
        table.insert(lines, "Answer:")
        for l in answer:gmatch("[^\n]+") do
            table.insert(lines, l)
        end
        table.insert(lines, string.rep("-", 40))
...
vim.api.nvim_create_user_command("AnkiReview", function(opts)
    ...
    complete = function()
        local api = require("anki.api")
        local ok, decks = pcall(api.deckNames)
        if ok and decks then
            return decks
        end
        return {}
    end,
